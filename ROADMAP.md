...
- **六个端到端样本** `prototypes/` — `rust-learn`(15) / `linear-algebra-learn`(13) / `english-reading-learn`(12) / `weight-loss-learn`(13) / `skincare-learn`(13) / `fitness-learn`(13)，均 novice→competent、三层齐全、DAG 经引擎校验；`weight-loss-learn` / `skincare-learn` / `fitness-learn` 是非认知（生理+行为）技能，演示框架的「诚实降级」（医学/动作/自报转线下）
