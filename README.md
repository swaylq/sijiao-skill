...
> 📊 &nbsp;**v0.1** — 元 skill + 工具引擎（**35 测试全绿**，纯标准库零依赖）+ **5 个端到端样本**（rust / 线性代数 / 英语阅读 / 减肥 / 皮肤管理，共 **66 模块**）跑通。每个样本的先修图都经引擎拓扑校验、课程由引擎渲染。
>
...
**本仓 5 个样本的硬数字**：66 模块全部经引擎拓扑校验、课程由引擎渲染、`learner-state` 示例 schema 验证通过，35 个测试覆盖工具引擎。
...
| **皮肤管理（科学护肤）** | 生理 · 行为（诚实降级 · ⚠️ 非医疗建议） | 13 | 胜任 | [skincare-learn/](prototypes/skincare-learn/) |

> 甜区起步（认知/知识型先跑通）；**减肥 / 皮肤管理是非认知技能**（生理/行为），展示框架碰到这类技能怎么「诚实降级」——AI 教不了的（实际执行 / 身体状况 / 医学情况）标注转自报 / 线下 / 医生。<br>
...
├── prototypes/               # 5 个端到端样本：rust / 线性代数 / 英语阅读 / 减肥 / 皮肤管理（66 模块）
└── tests/                    # 35 个测试
```
...
| 版本 | 内容 | 状态 |
|------|------|------|
| v0.1 | 元 skill + 工具引擎 + **5 个样本**（rust / 线性代数 / 英语阅读 / 减肥 / 皮肤管理）+ 打包 | ✅ |
